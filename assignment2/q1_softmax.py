...
def softmax(x):
    ...
    ### YOUR CODE HERE
    x -= tf.reduce_max(x)  # For numerical stability
    unNorm_probs = tf.exp(x)  # unnormalized probabilities
    out = unNorm_probs / tf.reduce_sum(unNorm_probs, axis=1, keep_dims=True)
    ### END YOUR CODE
